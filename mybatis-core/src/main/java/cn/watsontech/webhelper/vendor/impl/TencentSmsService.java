...
import cn.watsontech.webhelper.vendor.SmsService;
...
public class TencentSmsService extends SmsService {
    ...
    private SmsSingleSenderResult sendTencentRequest(TencentSmsSender smsSender, String smsSign, String templateId, String mobile, String[] templateDatas, String ext) {
        ...
        try {
            ...
            if(result.result==0) {
                //请求成功
                log.info("发送短信({})成功, 短信参数：{}", mobile, StringUtils.arrayToCommaDelimitedString(templateDatas));
                return result;
            }
    ...
    private List<SmsMultiSenderResult.Detail> sendTencentMultipleRequest(TencentSmsSender smsSender, String smsSign, String templateId, String[] mobiles, String[] templateDatas, String ext) {
        ...
        try {
            ...
            if(result.result==0) {
                //请求成功
                log.info("发送短信({})成功, 短信参数：{}", StringUtils.arrayToCommaDelimitedString(mobiles), StringUtils.arrayToCommaDelimitedString(templateDatas));
                return result.details;
            }

            log.error("发送短信({})失败, 短信参数：{}", StringUtils.arrayToCommaDelimitedString(mobiles), StringUtils.arrayToCommaDelimitedString(templateDatas));
