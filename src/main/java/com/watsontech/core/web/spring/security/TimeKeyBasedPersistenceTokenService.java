package com.watsontech.core.web.spring.security;
...
public class TimeKeyBasedPersistenceTokenService implements TokenService, InitializingBean {
	...
	public Token verifyToken(String key) {
		...
		try {
			...
			log.error("Invalid token, number format exception {}", nfe.getMessage());
		...
		if (expireTime<new Date().getTime()) {
			log.info("Token expired, accountInfo = {}", extendedInfo.toString());
		...
		if (!expectedSha512Hex.equals(sha1Hex)) {
			log.info("Key verification failure, accountInfo = {}", extendedInfo.toString());
